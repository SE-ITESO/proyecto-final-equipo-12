...
uint8_t string_clean_mode[]="1)Limpio";
uint8_t string_fx_mode[]="2)Efectos";
...
void state_machine_one(void)
{
	...
	case SELECT_MODE:
		if(0U == counter)
		{
			...
			LCD_nokia_goto_xy(0,2);
			LCD_nokia_send_string(string_clean_mode);
			LCD_nokia_goto_xy(0,3);
			LCD_nokia_send_string(string_fx_mode);
